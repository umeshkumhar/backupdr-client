/*
Backup and DR Service: Management Console API Spec

This document defines the API for the Global Manager. All communication is done over HTTPS with UTF-8 encoding. JSON is the only supported format for both request and response payloads. <p></p>Please read <a href=\"https://cloud.google.com/backup-disaster-recovery/docs/api/RestAPIGeneralConcepts.pdf\">Management Console API General concept</a><p></p>To login, use the /session POST API below.<br></br>Then copy the resulting session_id from the output and click on the Authorize button on the top right. Paste the string \"Actifio \" followed by the session id into the form and click Authorize.<p></p>Login is not necessary for reading the rest of this API document. However, login will allow you to try the APIs out within this page.

API version: V11.0.4
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ApplianceUpdateLogsRest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ApplianceUpdateLogsRest{}

// ApplianceUpdateLogsRest struct for ApplianceUpdateLogsRest
type ApplianceUpdateLogsRest struct {
	Logs []string `json:"logs,omitempty"`
	Logfilename *string `json:"logfilename,omitempty"`
	// Unique ID for this object
	Id *string `json:"id,omitempty"`
	// URL to access this object
	Href *string `json:"href,omitempty"`
	// When this object was last synced from appliances (UNIX Epoch time in microseconds). It does not apply to local resources.
	Syncdate *int64 `json:"syncdate,omitempty"`
	// Optional flag to indicate if the information is out-of-date due to communication problems with appliances. It does not apply to local resources.
	Stale *bool `json:"stale,omitempty"`
}

// NewApplianceUpdateLogsRest instantiates a new ApplianceUpdateLogsRest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewApplianceUpdateLogsRest() *ApplianceUpdateLogsRest {
	this := ApplianceUpdateLogsRest{}
	return &this
}

// NewApplianceUpdateLogsRestWithDefaults instantiates a new ApplianceUpdateLogsRest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewApplianceUpdateLogsRestWithDefaults() *ApplianceUpdateLogsRest {
	this := ApplianceUpdateLogsRest{}
	return &this
}

// GetLogs returns the Logs field value if set, zero value otherwise.
func (o *ApplianceUpdateLogsRest) GetLogs() []string {
	if o == nil || IsNil(o.Logs) {
		var ret []string
		return ret
	}
	return o.Logs
}

// GetLogsOk returns a tuple with the Logs field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApplianceUpdateLogsRest) GetLogsOk() ([]string, bool) {
	if o == nil || IsNil(o.Logs) {
		return nil, false
	}
	return o.Logs, true
}

// HasLogs returns a boolean if a field has been set.
func (o *ApplianceUpdateLogsRest) HasLogs() bool {
	if o != nil && !IsNil(o.Logs) {
		return true
	}

	return false
}

// SetLogs gets a reference to the given []string and assigns it to the Logs field.
func (o *ApplianceUpdateLogsRest) SetLogs(v []string) {
	o.Logs = v
}

// GetLogfilename returns the Logfilename field value if set, zero value otherwise.
func (o *ApplianceUpdateLogsRest) GetLogfilename() string {
	if o == nil || IsNil(o.Logfilename) {
		var ret string
		return ret
	}
	return *o.Logfilename
}

// GetLogfilenameOk returns a tuple with the Logfilename field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApplianceUpdateLogsRest) GetLogfilenameOk() (*string, bool) {
	if o == nil || IsNil(o.Logfilename) {
		return nil, false
	}
	return o.Logfilename, true
}

// HasLogfilename returns a boolean if a field has been set.
func (o *ApplianceUpdateLogsRest) HasLogfilename() bool {
	if o != nil && !IsNil(o.Logfilename) {
		return true
	}

	return false
}

// SetLogfilename gets a reference to the given string and assigns it to the Logfilename field.
func (o *ApplianceUpdateLogsRest) SetLogfilename(v string) {
	o.Logfilename = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *ApplianceUpdateLogsRest) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApplianceUpdateLogsRest) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *ApplianceUpdateLogsRest) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *ApplianceUpdateLogsRest) SetId(v string) {
	o.Id = &v
}

// GetHref returns the Href field value if set, zero value otherwise.
func (o *ApplianceUpdateLogsRest) GetHref() string {
	if o == nil || IsNil(o.Href) {
		var ret string
		return ret
	}
	return *o.Href
}

// GetHrefOk returns a tuple with the Href field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApplianceUpdateLogsRest) GetHrefOk() (*string, bool) {
	if o == nil || IsNil(o.Href) {
		return nil, false
	}
	return o.Href, true
}

// HasHref returns a boolean if a field has been set.
func (o *ApplianceUpdateLogsRest) HasHref() bool {
	if o != nil && !IsNil(o.Href) {
		return true
	}

	return false
}

// SetHref gets a reference to the given string and assigns it to the Href field.
func (o *ApplianceUpdateLogsRest) SetHref(v string) {
	o.Href = &v
}

// GetSyncdate returns the Syncdate field value if set, zero value otherwise.
func (o *ApplianceUpdateLogsRest) GetSyncdate() int64 {
	if o == nil || IsNil(o.Syncdate) {
		var ret int64
		return ret
	}
	return *o.Syncdate
}

// GetSyncdateOk returns a tuple with the Syncdate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApplianceUpdateLogsRest) GetSyncdateOk() (*int64, bool) {
	if o == nil || IsNil(o.Syncdate) {
		return nil, false
	}
	return o.Syncdate, true
}

// HasSyncdate returns a boolean if a field has been set.
func (o *ApplianceUpdateLogsRest) HasSyncdate() bool {
	if o != nil && !IsNil(o.Syncdate) {
		return true
	}

	return false
}

// SetSyncdate gets a reference to the given int64 and assigns it to the Syncdate field.
func (o *ApplianceUpdateLogsRest) SetSyncdate(v int64) {
	o.Syncdate = &v
}

// GetStale returns the Stale field value if set, zero value otherwise.
func (o *ApplianceUpdateLogsRest) GetStale() bool {
	if o == nil || IsNil(o.Stale) {
		var ret bool
		return ret
	}
	return *o.Stale
}

// GetStaleOk returns a tuple with the Stale field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApplianceUpdateLogsRest) GetStaleOk() (*bool, bool) {
	if o == nil || IsNil(o.Stale) {
		return nil, false
	}
	return o.Stale, true
}

// HasStale returns a boolean if a field has been set.
func (o *ApplianceUpdateLogsRest) HasStale() bool {
	if o != nil && !IsNil(o.Stale) {
		return true
	}

	return false
}

// SetStale gets a reference to the given bool and assigns it to the Stale field.
func (o *ApplianceUpdateLogsRest) SetStale(v bool) {
	o.Stale = &v
}

func (o ApplianceUpdateLogsRest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ApplianceUpdateLogsRest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Logs) {
		toSerialize["logs"] = o.Logs
	}
	if !IsNil(o.Logfilename) {
		toSerialize["logfilename"] = o.Logfilename
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Href) {
		toSerialize["href"] = o.Href
	}
	if !IsNil(o.Syncdate) {
		toSerialize["syncdate"] = o.Syncdate
	}
	if !IsNil(o.Stale) {
		toSerialize["stale"] = o.Stale
	}
	return toSerialize, nil
}

type NullableApplianceUpdateLogsRest struct {
	value *ApplianceUpdateLogsRest
	isSet bool
}

func (v NullableApplianceUpdateLogsRest) Get() *ApplianceUpdateLogsRest {
	return v.value
}

func (v *NullableApplianceUpdateLogsRest) Set(val *ApplianceUpdateLogsRest) {
	v.value = val
	v.isSet = true
}

func (v NullableApplianceUpdateLogsRest) IsSet() bool {
	return v.isSet
}

func (v *NullableApplianceUpdateLogsRest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableApplianceUpdateLogsRest(val *ApplianceUpdateLogsRest) *NullableApplianceUpdateLogsRest {
	return &NullableApplianceUpdateLogsRest{value: val, isSet: true}
}

func (v NullableApplianceUpdateLogsRest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableApplianceUpdateLogsRest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


