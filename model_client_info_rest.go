/*
Backup and DR Service: Management Console API Spec

This document defines the API for the Global Manager. All communication is done over HTTPS with UTF-8 encoding. JSON is the only supported format for both request and response payloads. <p></p>Please read <a href=\"https://cloud.google.com/backup-disaster-recovery/docs/api/RestAPIGeneralConcepts.pdf\">Management Console API General concept</a><p></p>To login, use the /session POST API below.<br></br>Then copy the resulting session_id from the output and click on the Authorize button on the top right. Paste the string \"Actifio \" followed by the session id into the form and click Authorize.<p></p>Login is not necessary for reading the rest of this API document. However, login will allow you to try the APIs out within this page.

API version: V11.0.4
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ClientInfoRest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ClientInfoRest{}

// ClientInfoRest struct for ClientInfoRest
type ClientInfoRest struct {
	Exportprop []NameValueRest `json:"exportprop,omitempty"`
	NasMountProp []NameValueRest `json:"nas-mount-prop,omitempty"`
	Host *string `json:"host,omitempty"`
	// Unique ID for this object
	Id *string `json:"id,omitempty"`
	// URL to access this object
	Href *string `json:"href,omitempty"`
	// When this object was last synced from appliances (UNIX Epoch time in microseconds). It does not apply to local resources.
	Syncdate *int64 `json:"syncdate,omitempty"`
	// Optional flag to indicate if the information is out-of-date due to communication problems with appliances. It does not apply to local resources.
	Stale *bool `json:"stale,omitempty"`
}

// NewClientInfoRest instantiates a new ClientInfoRest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewClientInfoRest() *ClientInfoRest {
	this := ClientInfoRest{}
	return &this
}

// NewClientInfoRestWithDefaults instantiates a new ClientInfoRest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewClientInfoRestWithDefaults() *ClientInfoRest {
	this := ClientInfoRest{}
	return &this
}

// GetExportprop returns the Exportprop field value if set, zero value otherwise.
func (o *ClientInfoRest) GetExportprop() []NameValueRest {
	if o == nil || IsNil(o.Exportprop) {
		var ret []NameValueRest
		return ret
	}
	return o.Exportprop
}

// GetExportpropOk returns a tuple with the Exportprop field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientInfoRest) GetExportpropOk() ([]NameValueRest, bool) {
	if o == nil || IsNil(o.Exportprop) {
		return nil, false
	}
	return o.Exportprop, true
}

// HasExportprop returns a boolean if a field has been set.
func (o *ClientInfoRest) HasExportprop() bool {
	if o != nil && !IsNil(o.Exportprop) {
		return true
	}

	return false
}

// SetExportprop gets a reference to the given []NameValueRest and assigns it to the Exportprop field.
func (o *ClientInfoRest) SetExportprop(v []NameValueRest) {
	o.Exportprop = v
}

// GetNasMountProp returns the NasMountProp field value if set, zero value otherwise.
func (o *ClientInfoRest) GetNasMountProp() []NameValueRest {
	if o == nil || IsNil(o.NasMountProp) {
		var ret []NameValueRest
		return ret
	}
	return o.NasMountProp
}

// GetNasMountPropOk returns a tuple with the NasMountProp field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientInfoRest) GetNasMountPropOk() ([]NameValueRest, bool) {
	if o == nil || IsNil(o.NasMountProp) {
		return nil, false
	}
	return o.NasMountProp, true
}

// HasNasMountProp returns a boolean if a field has been set.
func (o *ClientInfoRest) HasNasMountProp() bool {
	if o != nil && !IsNil(o.NasMountProp) {
		return true
	}

	return false
}

// SetNasMountProp gets a reference to the given []NameValueRest and assigns it to the NasMountProp field.
func (o *ClientInfoRest) SetNasMountProp(v []NameValueRest) {
	o.NasMountProp = v
}

// GetHost returns the Host field value if set, zero value otherwise.
func (o *ClientInfoRest) GetHost() string {
	if o == nil || IsNil(o.Host) {
		var ret string
		return ret
	}
	return *o.Host
}

// GetHostOk returns a tuple with the Host field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientInfoRest) GetHostOk() (*string, bool) {
	if o == nil || IsNil(o.Host) {
		return nil, false
	}
	return o.Host, true
}

// HasHost returns a boolean if a field has been set.
func (o *ClientInfoRest) HasHost() bool {
	if o != nil && !IsNil(o.Host) {
		return true
	}

	return false
}

// SetHost gets a reference to the given string and assigns it to the Host field.
func (o *ClientInfoRest) SetHost(v string) {
	o.Host = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *ClientInfoRest) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientInfoRest) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *ClientInfoRest) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *ClientInfoRest) SetId(v string) {
	o.Id = &v
}

// GetHref returns the Href field value if set, zero value otherwise.
func (o *ClientInfoRest) GetHref() string {
	if o == nil || IsNil(o.Href) {
		var ret string
		return ret
	}
	return *o.Href
}

// GetHrefOk returns a tuple with the Href field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientInfoRest) GetHrefOk() (*string, bool) {
	if o == nil || IsNil(o.Href) {
		return nil, false
	}
	return o.Href, true
}

// HasHref returns a boolean if a field has been set.
func (o *ClientInfoRest) HasHref() bool {
	if o != nil && !IsNil(o.Href) {
		return true
	}

	return false
}

// SetHref gets a reference to the given string and assigns it to the Href field.
func (o *ClientInfoRest) SetHref(v string) {
	o.Href = &v
}

// GetSyncdate returns the Syncdate field value if set, zero value otherwise.
func (o *ClientInfoRest) GetSyncdate() int64 {
	if o == nil || IsNil(o.Syncdate) {
		var ret int64
		return ret
	}
	return *o.Syncdate
}

// GetSyncdateOk returns a tuple with the Syncdate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientInfoRest) GetSyncdateOk() (*int64, bool) {
	if o == nil || IsNil(o.Syncdate) {
		return nil, false
	}
	return o.Syncdate, true
}

// HasSyncdate returns a boolean if a field has been set.
func (o *ClientInfoRest) HasSyncdate() bool {
	if o != nil && !IsNil(o.Syncdate) {
		return true
	}

	return false
}

// SetSyncdate gets a reference to the given int64 and assigns it to the Syncdate field.
func (o *ClientInfoRest) SetSyncdate(v int64) {
	o.Syncdate = &v
}

// GetStale returns the Stale field value if set, zero value otherwise.
func (o *ClientInfoRest) GetStale() bool {
	if o == nil || IsNil(o.Stale) {
		var ret bool
		return ret
	}
	return *o.Stale
}

// GetStaleOk returns a tuple with the Stale field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ClientInfoRest) GetStaleOk() (*bool, bool) {
	if o == nil || IsNil(o.Stale) {
		return nil, false
	}
	return o.Stale, true
}

// HasStale returns a boolean if a field has been set.
func (o *ClientInfoRest) HasStale() bool {
	if o != nil && !IsNil(o.Stale) {
		return true
	}

	return false
}

// SetStale gets a reference to the given bool and assigns it to the Stale field.
func (o *ClientInfoRest) SetStale(v bool) {
	o.Stale = &v
}

func (o ClientInfoRest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ClientInfoRest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Exportprop) {
		toSerialize["exportprop"] = o.Exportprop
	}
	if !IsNil(o.NasMountProp) {
		toSerialize["nas-mount-prop"] = o.NasMountProp
	}
	if !IsNil(o.Host) {
		toSerialize["host"] = o.Host
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Href) {
		toSerialize["href"] = o.Href
	}
	if !IsNil(o.Syncdate) {
		toSerialize["syncdate"] = o.Syncdate
	}
	if !IsNil(o.Stale) {
		toSerialize["stale"] = o.Stale
	}
	return toSerialize, nil
}

type NullableClientInfoRest struct {
	value *ClientInfoRest
	isSet bool
}

func (v NullableClientInfoRest) Get() *ClientInfoRest {
	return v.value
}

func (v *NullableClientInfoRest) Set(val *ClientInfoRest) {
	v.value = val
	v.isSet = true
}

func (v NullableClientInfoRest) IsSet() bool {
	return v.isSet
}

func (v *NullableClientInfoRest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableClientInfoRest(val *ClientInfoRest) *NullableClientInfoRest {
	return &NullableClientInfoRest{value: val, isSet: true}
}

func (v NullableClientInfoRest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableClientInfoRest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


