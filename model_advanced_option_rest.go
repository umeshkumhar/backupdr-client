/*
Backup and DR Service: Management Console API Spec

This document defines the API for the Global Manager. All communication is done over HTTPS with UTF-8 encoding. JSON is the only supported format for both request and response payloads. <p></p>Please read <a href=\"https://cloud.google.com/backup-disaster-recovery/docs/api/RestAPIGeneralConcepts.pdf\">Management Console API General concept</a><p></p>To login, use the /session POST API below.<br></br>Then copy the resulting session_id from the output and click on the Authorize button on the top right. Paste the string \"Actifio \" followed by the session id into the form and click Authorize.<p></p>Login is not necessary for reading the rest of this API document. However, login will allow you to try the APIs out within this page.

API version: V11.0.4
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the AdvancedOptionRest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AdvancedOptionRest{}

// AdvancedOptionRest struct for AdvancedOptionRest
type AdvancedOptionRest struct {
	Policy *PolicyRest `json:"policy,omitempty"`
	Name *string `json:"name,omitempty"`
	Value *string `json:"value,omitempty"`
	Slt *SltRest `json:"slt,omitempty"`
	Sla *SlaRest `json:"sla,omitempty"`
	App *ApplicationRest `json:"app,omitempty"`
	Various *bool `json:"various,omitempty"`
	// Unique ID for this object
	Id *string `json:"id,omitempty"`
	// URL to access this object
	Href *string `json:"href,omitempty"`
	// When this object was last synced from appliances (UNIX Epoch time in microseconds). It does not apply to local resources.
	Syncdate *int64 `json:"syncdate,omitempty"`
	// Optional flag to indicate if the information is out-of-date due to communication problems with appliances. It does not apply to local resources.
	Stale *bool `json:"stale,omitempty"`
}

// NewAdvancedOptionRest instantiates a new AdvancedOptionRest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAdvancedOptionRest() *AdvancedOptionRest {
	this := AdvancedOptionRest{}
	return &this
}

// NewAdvancedOptionRestWithDefaults instantiates a new AdvancedOptionRest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAdvancedOptionRestWithDefaults() *AdvancedOptionRest {
	this := AdvancedOptionRest{}
	return &this
}

// GetPolicy returns the Policy field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetPolicy() PolicyRest {
	if o == nil || IsNil(o.Policy) {
		var ret PolicyRest
		return ret
	}
	return *o.Policy
}

// GetPolicyOk returns a tuple with the Policy field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetPolicyOk() (*PolicyRest, bool) {
	if o == nil || IsNil(o.Policy) {
		return nil, false
	}
	return o.Policy, true
}

// HasPolicy returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasPolicy() bool {
	if o != nil && !IsNil(o.Policy) {
		return true
	}

	return false
}

// SetPolicy gets a reference to the given PolicyRest and assigns it to the Policy field.
func (o *AdvancedOptionRest) SetPolicy(v PolicyRest) {
	o.Policy = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *AdvancedOptionRest) SetName(v string) {
	o.Name = &v
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetValue() string {
	if o == nil || IsNil(o.Value) {
		var ret string
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetValueOk() (*string, bool) {
	if o == nil || IsNil(o.Value) {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasValue() bool {
	if o != nil && !IsNil(o.Value) {
		return true
	}

	return false
}

// SetValue gets a reference to the given string and assigns it to the Value field.
func (o *AdvancedOptionRest) SetValue(v string) {
	o.Value = &v
}

// GetSlt returns the Slt field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetSlt() SltRest {
	if o == nil || IsNil(o.Slt) {
		var ret SltRest
		return ret
	}
	return *o.Slt
}

// GetSltOk returns a tuple with the Slt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetSltOk() (*SltRest, bool) {
	if o == nil || IsNil(o.Slt) {
		return nil, false
	}
	return o.Slt, true
}

// HasSlt returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasSlt() bool {
	if o != nil && !IsNil(o.Slt) {
		return true
	}

	return false
}

// SetSlt gets a reference to the given SltRest and assigns it to the Slt field.
func (o *AdvancedOptionRest) SetSlt(v SltRest) {
	o.Slt = &v
}

// GetSla returns the Sla field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetSla() SlaRest {
	if o == nil || IsNil(o.Sla) {
		var ret SlaRest
		return ret
	}
	return *o.Sla
}

// GetSlaOk returns a tuple with the Sla field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetSlaOk() (*SlaRest, bool) {
	if o == nil || IsNil(o.Sla) {
		return nil, false
	}
	return o.Sla, true
}

// HasSla returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasSla() bool {
	if o != nil && !IsNil(o.Sla) {
		return true
	}

	return false
}

// SetSla gets a reference to the given SlaRest and assigns it to the Sla field.
func (o *AdvancedOptionRest) SetSla(v SlaRest) {
	o.Sla = &v
}

// GetApp returns the App field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetApp() ApplicationRest {
	if o == nil || IsNil(o.App) {
		var ret ApplicationRest
		return ret
	}
	return *o.App
}

// GetAppOk returns a tuple with the App field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetAppOk() (*ApplicationRest, bool) {
	if o == nil || IsNil(o.App) {
		return nil, false
	}
	return o.App, true
}

// HasApp returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasApp() bool {
	if o != nil && !IsNil(o.App) {
		return true
	}

	return false
}

// SetApp gets a reference to the given ApplicationRest and assigns it to the App field.
func (o *AdvancedOptionRest) SetApp(v ApplicationRest) {
	o.App = &v
}

// GetVarious returns the Various field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetVarious() bool {
	if o == nil || IsNil(o.Various) {
		var ret bool
		return ret
	}
	return *o.Various
}

// GetVariousOk returns a tuple with the Various field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetVariousOk() (*bool, bool) {
	if o == nil || IsNil(o.Various) {
		return nil, false
	}
	return o.Various, true
}

// HasVarious returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasVarious() bool {
	if o != nil && !IsNil(o.Various) {
		return true
	}

	return false
}

// SetVarious gets a reference to the given bool and assigns it to the Various field.
func (o *AdvancedOptionRest) SetVarious(v bool) {
	o.Various = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *AdvancedOptionRest) SetId(v string) {
	o.Id = &v
}

// GetHref returns the Href field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetHref() string {
	if o == nil || IsNil(o.Href) {
		var ret string
		return ret
	}
	return *o.Href
}

// GetHrefOk returns a tuple with the Href field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetHrefOk() (*string, bool) {
	if o == nil || IsNil(o.Href) {
		return nil, false
	}
	return o.Href, true
}

// HasHref returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasHref() bool {
	if o != nil && !IsNil(o.Href) {
		return true
	}

	return false
}

// SetHref gets a reference to the given string and assigns it to the Href field.
func (o *AdvancedOptionRest) SetHref(v string) {
	o.Href = &v
}

// GetSyncdate returns the Syncdate field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetSyncdate() int64 {
	if o == nil || IsNil(o.Syncdate) {
		var ret int64
		return ret
	}
	return *o.Syncdate
}

// GetSyncdateOk returns a tuple with the Syncdate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetSyncdateOk() (*int64, bool) {
	if o == nil || IsNil(o.Syncdate) {
		return nil, false
	}
	return o.Syncdate, true
}

// HasSyncdate returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasSyncdate() bool {
	if o != nil && !IsNil(o.Syncdate) {
		return true
	}

	return false
}

// SetSyncdate gets a reference to the given int64 and assigns it to the Syncdate field.
func (o *AdvancedOptionRest) SetSyncdate(v int64) {
	o.Syncdate = &v
}

// GetStale returns the Stale field value if set, zero value otherwise.
func (o *AdvancedOptionRest) GetStale() bool {
	if o == nil || IsNil(o.Stale) {
		var ret bool
		return ret
	}
	return *o.Stale
}

// GetStaleOk returns a tuple with the Stale field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvancedOptionRest) GetStaleOk() (*bool, bool) {
	if o == nil || IsNil(o.Stale) {
		return nil, false
	}
	return o.Stale, true
}

// HasStale returns a boolean if a field has been set.
func (o *AdvancedOptionRest) HasStale() bool {
	if o != nil && !IsNil(o.Stale) {
		return true
	}

	return false
}

// SetStale gets a reference to the given bool and assigns it to the Stale field.
func (o *AdvancedOptionRest) SetStale(v bool) {
	o.Stale = &v
}

func (o AdvancedOptionRest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AdvancedOptionRest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Policy) {
		toSerialize["policy"] = o.Policy
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Value) {
		toSerialize["value"] = o.Value
	}
	if !IsNil(o.Slt) {
		toSerialize["slt"] = o.Slt
	}
	if !IsNil(o.Sla) {
		toSerialize["sla"] = o.Sla
	}
	if !IsNil(o.App) {
		toSerialize["app"] = o.App
	}
	if !IsNil(o.Various) {
		toSerialize["various"] = o.Various
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Href) {
		toSerialize["href"] = o.Href
	}
	if !IsNil(o.Syncdate) {
		toSerialize["syncdate"] = o.Syncdate
	}
	if !IsNil(o.Stale) {
		toSerialize["stale"] = o.Stale
	}
	return toSerialize, nil
}

type NullableAdvancedOptionRest struct {
	value *AdvancedOptionRest
	isSet bool
}

func (v NullableAdvancedOptionRest) Get() *AdvancedOptionRest {
	return v.value
}

func (v *NullableAdvancedOptionRest) Set(val *AdvancedOptionRest) {
	v.value = val
	v.isSet = true
}

func (v NullableAdvancedOptionRest) IsSet() bool {
	return v.isSet
}

func (v *NullableAdvancedOptionRest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAdvancedOptionRest(val *AdvancedOptionRest) *NullableAdvancedOptionRest {
	return &NullableAdvancedOptionRest{value: val, isSet: true}
}

func (v NullableAdvancedOptionRest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAdvancedOptionRest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


